...
class URLGenerator:
  ...
  def generate(self):
    f1 = open('archive/processed/ScienceYear.txt', 'r')
    f2 = open('archive/processed/Science.txt', 'w')
    pool = f1.readlines()
    for url in pool:
      time.sleep(5)
      year = url.split('/')
  ...
  def synthesis(self):
    f = open('archive/processed/Science.txt', 'w')
    parentURL = "http://www.sciencemag.org/content/by/year/"
